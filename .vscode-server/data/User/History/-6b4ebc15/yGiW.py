...
class RealsenseCamSuber(Node):
    '''subscribe the message of realsense camera'''
    def __init__(self, name) -> None:
        super().__init__(name)
        self.bridge = CvBridge()
        self.declare_parameter("dog_name", "az2")
        self.sub = self.create_subscription(Image, '/az2/camera/infra1/image_rect_raw', self.sub_callback, 10)
        self.frame_count = 0
        self.size = 0
        self.tele_position = (0, 0)

    def sub_callback(self, msg: Image):
        '''the callback function of subscriber'''
        depth_msg = msg
        cv_image = self.bridge.imgmsg_to_cv2(depth_msg, "mono8")
