# -*- coding: utf-8 -*-
'''
> move_horizontal
> author: zzr
> date: 2024-05-24
> make the dog move horizontal aiming at the ball
> note:
    - if speed.y > 0 the dog move left,if speed.y < 0 the dog move right
    - if ball_y > 330, the ball is close!

'''

import cv2
from cv_bridge import CvBridge
import numpy as np
import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image, Range
from protocol.msg import MotionServoCmd
import threading
import time
# import socket

from .rgb_cam_suber import RGBCamSuber
from .side_cam import SideCam

class move_horizontal(Node):
    def __init__(self, name, RGBCamSuber):
        super().__init__(name)
        self.rgb_node = RGBCamSuber
        # self.location = Location()
        self.speed_x, self.speed_y, self.speed_z = 0.0, 0.0, 0.0
        self.dog_name = "az2"
        self.pub = self.create_publisher(MotionServoCmd, f"/{self.dog_name}/motion_servo_cmd", 10)
        self.timer = self.create_timer(0.1, self.timer_callback)
        self.x_rec=[320.0,320.0,320.0]  # 优化：none类型
        self.avx_rec=[320.0,320.0,320.0]
        self.size_rec=[.0,.0,.0,.0,.0]
        self.aim = False


    def timer_callback(self):
        msg = MotionServoCmd()
        rclpy.spin_once(self.rgb_node)
        ball_x, ball_y = self.rgb_node.ball_position
        size = self.rgb_node.size

        # 持续更新x坐标，size，avx
        self.x_rec.pop(0)
        self.x_rec.append(ball_x)
        self.size_rec.pop(0)
        self.size_rec.append(size)

        avx=sum(self.x_rec)/len(self.x_rec)  
        avs=sum(self.size_rec)/len(self.size_rec)

        self.avx_rec.pop(0)
        self.avx_rec.append(avx)

        ball_x_vel = (self.avx_rec[2]-self.avx_rec[1])  # velocity 

        # 优化：利用左右相机，如果球在左边就疯狂往左走，反之亦然，除非size很小
        # 问题：如果球离狗很近，但是球在视野边缘，size也会很小，是否可以用深度相机或者激光雷达？y>330为界限
        # 狗的前后位置校准、角度校准
        # 情况0：一般射门
        # 情况1：绕右，然后向左射门（近距离大角度射门）--让狗左右移动不能过大；

        # 如果球几乎在眼前 阶段4
        if (avs >= 3000) | (ball_y > 350): 
            msg.motion_id = 305 
            if avx > 320:
                self.speed_x, self.speed_y, self.speed_z = 0.0, -0.5, 0.0
            else:
                self.speed_x, self.speed_y, self.speed_z = 0.0, 0.5, 0.0

        # 如果球比较近了 阶段3
        if (avs >= 1700) & (avs < 3000): 
            msg.motion_id = 305 
            if ball_x_vel > 0: # 如果x越来越大，狗往右走
                if ball_x_vel < 15:
                    self.speed_x, self.speed_y, self.speed_z = 0.0, -0.20, 0.0
                elif (ball_x_vel >= 15) & (ball_x_vel < 50):
                    self.speed_x, self.speed_y, self.speed_z = 0.0, -0.30, 0.0
                else:
                    self.speed_x, self.speed_y, self.speed_z = 0.0, -0.40, 0.0
            else:
                if ball_x_vel > -15:
                    self.speed_x, self.speed_y, self.speed_z = 0.0, 0.20, 0.0
                elif (ball_x_vel <= -15) & (ball_x_vel > -50):
                    self.speed_x, self.speed_y, self.speed_z = 0.0, 0.30, 0.0
                else:
                    self.speed_x, self.speed_y, self.speed_z = 0.0, 0.40, 0.0

            # if avx > 280 and avx < 360:  #球在视野中心：不再平移
            #     self.speed_x, self.speed_y, self.speed_z = 0.0, 0.0, 0.0
            #     self.aim = True
            # else:
            #     if (self.x_rec[0]+2.0 < self.x_rec[1]) & (self.x_rec[1]+2.0 < self.x_rec[2]): # 如果x越来越大，狗往右走
            #         self.speed_x, self.speed_y, self.speed_z = 0.0, -0.30, 0.0
            #     elif (self.x_rec[2]+2.0 < self.x_rec[1]) & (self.x_rec[1]+2.0 < self.x_rec[0]):# 如果x越来越小，狗往左走
            #         self.speed_x, self.speed_y, self.speed_z = 0.0, 0.30, 0.0
            #     else: # 如果球几乎不动，狗不动
            #         self.speed_x, self.speed_y, self.speed_z = 0.0, 0.0, 0.0
            #         self.aim = True

        # 如果球很远或者走掉了 阶段2
        if (avs > 100) & (avs < 1700): 
            msg.motion_id = 308
            if  avx < 280: #球从左侧溜走:向左走  320是像素中心位置
                self.speed_x, self.speed_y, self.speed_z = 0.0, 0.27, 0.0
                if avx > 300:
                    self.speed_x, self.speed_y, self.speed_z = 0.0, 0.0, 0.0
            elif avx > 360: # 球从右侧溜走：向右走
                self.speed_x, self.speed_y, self.speed_z = 0.0, -0.27, 0.0
                if avx < 340:
                    self.speed_x, self.speed_y, self.speed_z = 0.0, 0.0, 0.0
            else:
                self.speed_x, self.speed_y, self.speed_z = 0.0, 0.0, 0.0

        # 如果球不见了或者球在往外走 阶段1
        if (avs <= 100):  
            msg.motion_id = 308
            self.speed_x, self.speed_y, self.speed_z = 0.0, 0.0, 0.0
            self.aim = False


        msg.cmd_type = 1
        msg.value = 2
        msg.vel_des = [self.speed_x, self.speed_y, self.speed_z]
        msg.step_height = [0.05, 0.05]
        self.pub.publish(msg)
        self.get_logger().info(f"avs={avs},avx={avx},velx={ball_x_vel},by={ball_y}")


def move_horizontal_aim_ball(args=None):
    rclpy.init()
    rgb_node = RGBCamSuber("RGBCamSuber")
    move_horizontal_node = move_horizontal("move_horizontal_node", rgb_node)
    move_horizontal_thread = threading.Thread(target=rclpy.spin, args=(move_horizontal_node,))
    move_horizontal_thread.start()
    try:
        while True:
            pass
    except KeyboardInterrupt:
        print('KeyBoardInterrupt')
    move_horizontal_node.destroy_node()
    rgb_node.destroy_node()
    rclpy.shutdown()
    cv2.destroyAllWindows()
    return True


def main(args=None):
    print(move_horizontal_aim_ball())


if __name__ == '__main__':
    main()